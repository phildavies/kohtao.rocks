...
( function( $ ) {
    ...
    $( function() {
        $( document ).ready( function() {
            $('.featured-content-wrapper .entry-container, .portfolio-content-wrapper .entry-container, .testimonial-content-wrapper .entry-content').matchHeight();
        });
    });
    ...
    masthead         = $( '#masthead' );
    menuToggle       = masthead.find( '#primary-menu-toggle' ); // button id
    siteHeaderMenu   = masthead.find( '#site-header-right' ); // wrapper id
    siteNavigation   = masthead.find( '#site-navigation' ); // nav id
    ...
    ( function() {
        ...
        menuToggle.on( 'click.divin', function() {
    ...
    ( function() {
        ...
        function toggleFocusClassTouchScreen() {
            if ( window.innerWidth >= 910 ) {
                $( document.body ).on( 'touchstart.divin', function( e ) {
                    if ( ! $( e.target ).closest( '.main-navigation li' ).length ) {
                        $( '.main-navigation li' ).removeClass( 'focus' );
                    }
                } );
                siteNavigation.find( '.menu-item-has-children > a' ).on( 'touchstart.divin', function( e ) {
                ...
                siteNavigation.find( '.menu-item-has-children > a' ).unbind( 'touchstart.divin' );
            }
        }

        if ( 'ontouchstart' in window ) {
            $( window ).on( 'resize.divin', toggleFocusClassTouchScreen );
            toggleFocusClassTouchScreen();
        }

        siteNavigation.find( 'a' ).on( 'focus.divin blur.divin', function() {
