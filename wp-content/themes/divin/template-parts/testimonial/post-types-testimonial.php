<?php
/**
 * The template for displaying testimonial items
 *
 * @package Divin
 */
?>

<?php
$number = get_theme_mod( 'divin_testimonial_number', 3 );

if ( ! $number ) {
	// If number is 0, then this section is disabled
	return;
}

$args = array(
	'orderby'             => 'post__in',
	'ignore_sticky_posts' => 1 // ignore sticky posts
);

$post_list  = array();// list of valid post/page ids

$no_of_post = 0; // for number of posts

$args['post_type'] = 'jetpack-testimonial';

for ( $i = 1; $i <= $number; $i++ ) {
	$divin_post_id =  get_theme_mod( 'divin_testimonial_cpt_' . $i );

	if ( $divin_post_id && '' !== $divin_post_id ) {
		// Polylang Support.
		if ( class_exists( 'Polylang' ) ) {
			$divin_post_id = pll_get_post( $divin_post_id, pll_current_language() );
		}

		$post_list = array_merge( $post_list, array( $divin_post_id ) );

		$no_of_post++;
	}
}

$args['post__in'] = $post_list;

if ( 0 === $no_of_post ) {
	return;
}

$args['posts_per_page'] = $no_of_post;
$loop     = new WP_Query( $args );

$slider_select = get_theme_mod( 'divin_testimonial_slider' );

$layouts = 1;
$layout = get_theme_mod( 'divin_testimonial_layout', 'layout-three' );

if ( 'layout-two' === $layout ) {
	$layouts = 2;
}
elseif ( 'layout-three' === $layout ) {
	$layouts = 3;
}
elseif ( 'layout-four' === $layout ) {
	$layouts = 4;
}

if ( $loop -> have_posts() ) :
	while ( $loop -> have_posts() ) :
		$loop -> the_post();

		get_template_part( 'template-parts/testimonial/content', 'testimonial' );

		$i = absint( $loop->current_post + 1 );

		//end and start testimonial_slider_wrap div based on logic
		if ( 0 === ( $i % $layouts ) && $i < $no_of_post && $slider_select ) : ?>
			</div><!-- .testimonial_slider_wrap -->

			<div class="testimonial_slider_wrap">
		<?php
		endif;
	endwhile;
	wp_reset_postdata();
endif;
